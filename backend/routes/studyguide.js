...
//get and post routes for unique ids and usernames, usually depending on what params are available
//verifyToken used for user verification
...
router.get("/byid/:id/cards", async (req, res, next) => {
  try {
    ...
    if (!studyguide) {
      return res
        .status(404)
        .json({ message: `No Study Guide found: ${req.params.id}` });
    }
...
router.put("/byid/:id/:cardid", verifyToken, async (req, res, next) => {
  try {
    ...
    console.log(req.body);
    console.log(id);
    const newcard = await FlashCards.findByIdAndUpdate(id, req.body, {
      new: true,
    });
    res.send({
      newcard,
